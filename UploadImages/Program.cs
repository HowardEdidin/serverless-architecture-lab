...
namespace UploadImages
{
	internal class Program
	{
		...
		private static string GenerateRandomFileName()
		{
			...
			var rando = new string(Enumerable.Repeat(chars, randomStringLength).Select(s => s[Random.Next(s.Length)]).ToArray());
			return $"{rando}.jpg";
		}
		...
		private static void LoadImagesFromDisk(bool upload1000)
		{
			...
			if (upload1000)
				_sourceImages =
					Directory.GetFiles(@"..\..\..\..\license plates\copyfrom\").Select(f => new MemoryStream(File.ReadAllBytes(f))).
						ToList();
			else
				_sourceImages =
					Directory.GetFiles(@"..\..\..\..\license plates\").Select(f => new MemoryStream(File.ReadAllBytes(f))).ToList();
		}
	}
}