	...
	public class FindLicensePlateText
	{
		...
		private async Task<string> MakeOcrRequest(byte[] imageBytes)
		{
			...
			try
			{
				...
				var response = await resiliencyStrategy.ExecuteAsync(() => client.PostAsync(uri, GetImageHttpContent(imageBytes)));
			...
			{
				log.Error($"Could not contact the Computer Vision API service due to the following error: {bce.Message}");
			}
			catch (Exception e)
			{
			...
			log.Info($"Finished OCR request. Result: {licensePlate}");
		...
		private PolicyWrap<HttpResponseMessage> DefineAndRetrieveResiliencyStrategy()
		{
			...
			var waitAndRetryPolicy = Policy.Handle<HttpRequestException>().OrResult<HttpResponseMessage>(e =>
				e.StatusCode == HttpStatusCode.ServiceUnavailable ||
				e.StatusCode == (HttpStatusCode) 429 || e.StatusCode == (HttpStatusCode) 403).WaitAndRetryAsync(
				...
				{
					log.Info(
						$"Computer Vision API server is throttling our requests. Automatically delaying for {calculatedWaitDuration.TotalMilliseconds}ms");
				}
			);
			...
			var circuitBreakerPolicyForRecoverable = Policy.Handle<HttpResponseException>().
				OrResult<HttpResponseMessage>(r => httpStatusCodesWorthRetrying.Contains(r.StatusCode)).CircuitBreakerAsync(
					3,
					TimeSpan.FromSeconds(3),
					(outcome, breakDelay) =>
					{
						log.Info(
							$"Polly Circuit Breaker logging: Breaking the circuit for {breakDelay.TotalMilliseconds}ms due to: {outcome.Exception?.Message ?? outcome.Result.StatusCode.ToString()}");
					},
					() => log.Info("Polly Circuit Breaker logging: Call ok... closed the circuit again"),
					() => log.Info("Polly Circuit Breaker logging: Half-open: Next call is a trial")
				);
